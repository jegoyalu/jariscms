 ...
 */
    public static function add(array $fields, string $type): bool
    {
        if ($type == "") {
            return false;
        }
     ...
     * @return bool True on success or false on failure.
     */
    public static function edit(int $id, array $fields, string $type): bool
    {
        if ($id == "" && $type == "") {
            return false;
        }
     ...
     */
    public static function get(int $id, string $type): array
    {
        if ($id == "" && $type == "") {
            return [];
        }
     ...
     */
    public static function appendFields(string $type, ?array &$current_fields): void
    {
        $fields = self::getList($type);
     ...
     * @return bool
     */
    public static function validUploads(string $type): bool
    {
        $fields = self::getList($type);
        ...
        foreach ($fields as $id => $field_data) {
            ...
            if ($field_data["type"] == "file") {
                ...
                if ($field_data["file_multiple"]) {
                    ...
                    $files = [
                    "name" => [$_FILES[$field_data["variable_name"]]["name"]],
                    "type" => [$_FILES[$field_data["variable_name"]]["type"]],
                    "tmp_name" => [$_FILES[$field_data["variable_name"]]["tmp_name"]]
                ];
                }
     ...
     */
    public static function saveUploads(string $type, string $page): void
    {
        $fields = self::getList($type);
     ...
     */
    public static function generateFields(string $type, array $values = []): array
    {
        if ($type == "") {
            return [];
        }
            ...
            $field["type"] == "uriarea"
                ...
                if ($values[$field["variable_name"]] != "") {
                    ...
                    foreach ($images as $image) {
                        ...
                        . "\">"
                        . "<img src=\""
                        . Uri::url("image/{$_REQUEST["uri"]}/$image", ["w" => 250])
                        . "\" />"
                        . "</a>"
                        . "</td>"
                    ;
