 ...
 */
static function startIfUserLogged(): bool
{
 ...
 */
static function destroyIfEmpty(): void
{
    if(self::$session_started)
    {
        if(empty($_SESSION))
 ...
 * @return array
 */
static function getCookieParams(): array
{
 ...
 * @param string  $path
 */
static function addCookie(
    string $name, $value, int $expires=0, string $path="/"
): void
{
    if(is_array($value))
    {
 ...
 * @param string  $path
 */
static function removeCookie(string $name, string $path="/")
{
    $cookie_params = self::getCookieParams();
