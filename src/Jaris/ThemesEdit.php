 ...
 * @return array
 */
    public static function settings(string $theme, bool $expanded=false): array
    {
     ...
     */
    public static function saveSettings(array $settings, string $theme):bool
    {
        $theme_path = Themes::directory($theme);
     ...
     */
    public static function getFileFields(string $theme): array
    {
     ...
     */
    public static function generateForm(string $theme): string
    {
        if (
        ...
        self::validUploads($theme)
            ...
            Uri::go("admin/themes/view", ["path" => $_REQUEST["path"]]);
        }
        ...
        $fieldsets[] = ["fields" => $fields_submit];
            ...
            foreach ($fieldset["fields"] as $id => $field_data) {
                if ($field_data["type"] == "file") {
                    ...
                    if ($field_data["multiple"]) {
                        ...
                        $files = [
                        "name" => [$_FILES[$field_data["name"]]["name"]],
                        "type" => [$_FILES[$field_data["name"]]["type"]],
                        "tmp_name" => [$_FILES[$field_data["name"]]["tmp_name"]]
                    ];
                    }
