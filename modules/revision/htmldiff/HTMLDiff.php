...
class DomTreeBuilder
{
    ...
    public function characters($parser, $data)
    {
        ...
        foreach ($matches as &$word) {
            ...
            } elseif (preg_match(self::delimiter, $word)) {
                $this->endWord();
    ...
    {
        return array_map(['TextNode','toDiffLine'], $this->textNodes);
    }
}
...
class TextNodeDiffer
{
    ...
    public function __construct(DomTreeBuilder $tree, DomTreeBuilder $oldTree)
    {
    ...
    public function handlePossibleChangedPart($leftstart, $leftend, $rightstart, $rightend)
    {
        ...
        while ($i < $rightend) {
            ...
            if ($result) {
                ...
                if (!$this->changedIDUsed) {
                    $mod->firstOfID = true;
                } elseif (!is_null($result) && $result !== $this->changes) {
                    ++$this->changedID;
                    $mod->firstOfID = true;
                }
    ...
    public function markAsDeleted($start, $end, $before)
    {
        ...
        while (count($deletedNodes) > 0) {
            ...
            } elseif ($prevResult->lastCommonParentDepth < $nextResult->lastCommonParentDepth) {
...
class HTMLDiffer
{
    ...
    public function htmlDiff($from, $to, $describe_formatting_changes = true)
    {
        ...
        xml_set_element_handler($xml_parser, [$domfrom, "startElement"], [$domfrom, "endElement"]);

        // Set the function to handle blocks of character data
        xml_set_character_data_handler($xml_parser, [$domfrom, "characters"]);
        ...
        // Set the functions to handle opening and closing tags
        xml_set_element_handler($xml_parser, [$domto, "startElement"], [$domto, "endElement"]);

        // Set the function to handle blocks of character data
        xml_set_character_data_handler($xml_parser, [$domto, "characters"]);
        ...
        foreach ($differences as &$d) {
            if ($d->leftstart > $currentIndexLeft && $describe_formatting_changes) {
                $domdiffer->handlePossibleChangedPart(
                    $currentIndexLeft,
                    $d->leftstart,
                    $currentIndexRight,
                    $d->rightstart
    ...
    private function preProcess(/*array*/ $differences)
    {
        ...
        for ($i = 0; $i < $nbDifferences; ++$i) {
            ...
            while ($i + 1 < $nbDifferences && self::score(
                $leftLength,
...
class TextOnlyComparator
{
    ...
    public function _construct(TagNode $tree)
    {
        $this->addRecursive($tree);
        $this->leafs = array_map(['TextNode','toDiffLine'], $this->leafs);
    }
...
class AncestorComparator
{
    ...
    public function __construct(/*array*/ $ancestors)
    {
        ...
        $this->ancestorsText = array_map(['TagNode','toDiffLine'], $ancestors);
    }
...
{
    private static $containerTags = ['html', 'body', 'p', 'blockquote',
        'h1', 'h2', 'h3', 'h4', 'h5', 'pre', 'div', 'ul', 'ol', 'li',
        'table', 'tbody', 'tr', 'td', 'th', 'br', 'hr', 'code', 'dl',
        'dt', 'dd', 'input', 'form', 'img', 'span', 'a'];

    private static $styleTags = ['i', 'b', 'strong', 'em', 'font',
...
class TagToString
{
    ...
    public function __construct(TagNode $node, $sem)
    {
        $this->node = $node;
        $this->sem = $sem;
    }
    ...
    public function getRemovedDescription(ChangeText $txt)
    {
        ...
        } elseif ($this->sem == TagToStringFactory::STYLE) {
...
class HTMLOutput
{
    ...
    public function __construct($prefix, $handler)
    {
        $this->prefix = $prefix;
        ...
        foreach ($node->children as &$child) {
            if ($child instanceof TagNode) {
                ...
                if ($newStarted && ($mod->type != Modification::ADDED || $mod->firstOfID)) {
                    $handler->endElement('span');
                    $newStarted = false;
                } elseif ($changeStarted && ($mod->type != Modification::CHANGED
                        || $mod->changes != $changeTXT || $mod->firstOfID)) {
                    $handler->endElement('span');
                    $changeStarted = false;
                } elseif ($remStarted && ($mod->type != Modification::REMOVED || $mod ->firstOfID)) {
                    $handler->endElement('span');
                    $remStarted = false;
                }
                ...
                if (!$newStarted && $mod->type == Modification::ADDED) {
                    ...
                    //tooltip
                    $handler->startElement('span', ['class' => 'tip']);
                    $handler->html($mod->changes);
                    $handler->endElement('span');

                    $changeStarted = true;
                    $changeTXT = $mod->changes;
                } elseif (!$remStarted && $mod->type == Modification::REMOVED) {
...
class DelegatingContentHandler
{
    ...
    public function startElement($qname, /*array*/ $arguments)
    {
        $this->delegate->addHtml(Xml::openElement($qname, $arguments));
    }

    public function endElement($qname)
    {
