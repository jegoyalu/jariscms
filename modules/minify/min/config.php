 ...
 * Gets the cache dir
 */
function get_min_cache_dir($main_dir)
{
...
$min_documentRoot = str_replace(
    "/modules/minify/min/index.php",
    "",
    $_SERVER['DOCUMENT_ROOT'] . $_SERVER['PHP_SELF']
);

$min_documentRoot = preg_replace(
    "/\/sites\/(.*)/",
    "",
    $min_documentRoot
);

//Configuration to getting Minify to work in a subdirectory of the document root
list($__siteUri) = explode('/modules/minify/min/index.php', $_SERVER['SCRIPT_NAME'], 2);

$__siteUri = preg_replace(
    "/\/sites\/(.*)/",
    "",
    $__siteUri
);
 ...
 */
//$min_symlinks = array();
$min_symlinks['//' . ltrim($__siteUri, '/')] = $min_documentRoot; //So it works on subdirectories
