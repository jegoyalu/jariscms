...
row: 0
    field: title
        <?php print t("Content to Approve") ?>
    field;

    field: content
    <?php
        Jaris\Authentication::protectedPage(array("approve_content"));
    ?>
    <script>
    ...
    <?php
        ...
        print "<form method=\"get\" action=\"" . Jaris\Uri::url("admin/pages/approve") . "\">\n";
        ...
        $pages_count = Jaris\Sql::countColumn(
            ...
            "uris",
            "uri",
            "where approved='p' $where $type $author"
        );

        print "<form method=\"post\" action=\"" . Jaris\Uri::url("admin/pages/mass-approve") . "\">\n";
        print "<div style=\"float: left\">";
        ...
        print " <input type=\"submit\" value=\"" . t("Approve Selected") . "\">\n";
        print "</div>";
        ...
        $pages = Jaris\Sql::getDataList(
            ...
            $page - 1,
            20,
            "where approved='p' $where $type $author order by created_date desc"
        );
        ...
        foreach($pages as $result_fields)
        {
            ...
            if($image_url)
                print "<td><a target=\"_blank\" href=\"" . Jaris\Uri::url($uri) . "\"><img src=\"$image_url\" /></a></td>";
            else
                print "<td></td>";

            print "<td><a target=\"_blank\" href=\"" . Jaris\Uri::url($uri) . "\">" . Jaris\System::evalPHP($page_data["title"]) . "</a></td>";
            ...
            {
                print date("m/d/Y g:i:s a", $page_data["created_date"]);
            }
            print "</td>";
